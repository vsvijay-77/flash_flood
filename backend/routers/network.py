...
async def network_stats(user: dict = Depends(current_user)):
    ...
    active = await db.sensors.count_documents({"status": "online"})
    gateways = await db.gateways.count_documents({"status": "online"})
    alerts = await db.alerts.count_documents({"status": {"$ne": "resolved"}})
    zones = await db.zones.count_documents({})
    critical = await db.alerts.count_documents({"status": {"$ne": "resolved"}, "risk_level": "critical"})
    high = await db.alerts.count_documents({"status": {"$ne": "resolved"}, "risk_level": "high"})
